from Member.member import Member
#====================
# 회원 데이터 접근 (CRUD) : MemberDAO
class MemberDAO:
    def __init__(self):
        self.__memberDB: dict[str, Member] = {}  # id : Member

    def insert_member(self, member: Member) -> bool:
        if member.get_id() in self.__memberDB:
            return False
        self.__memberDB[member.get_id()] = member
        return True

    def select_member_by_id(self, id: str) -> Member | None:
        return self.__memberDB.get(id)

    def select_all_members(self) -> list[Member]:
        # 컬렉션 조회는 없을 때 None 이 아니라 빈 리스트를 반환한다.
        return list(self.__memberDB.values())

    def update_member(self, id: str, member: Member) -> bool:
        if id in self.__memberDB:
            self.__memberDB[id] = member
            return True
        return False

    def delete_member(self, id: str) -> bool:
        if id in self.__memberDB:
            self.__memberDB.pop(id)
            return True
        return False

# 단위테스트
if __name__ == '__main__':
    dao = MemberDAO()
    dao.insert_member(Member('woongseok', '1234', '최웅석', 'a@a.com', '010'))
    print(dao.select_member_by_id('woongseok'))
    print(dao.insert_member(Member('woongseok', '1', 'dup', 'b', 'c')))  # False
    print(dao.select_all_members())
    print(dao.delete_member('woongseok'))
