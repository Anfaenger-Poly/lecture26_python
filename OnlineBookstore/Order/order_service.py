...
class OrderService:
    ...
    def create_order(self, member_id, book_id, quantity):
        if quantity <= 0:
            raise InvalidInputError('수량은 1 이상이어야 합니다.')
        book = self.__book_dao.select_book_by_id(book_id)
        if not book:
            raise NotFoundError(f'없는 도서입니다. (도서 번호 {book_id})')
        if book.get_stock() < quantity:
            raise OutOfStockError(f'재고가 부족합니다. (현재 재고 {book.get_stock()})')
        # 재고 차감
        book.set_stock(book.get_stock() - quantity)
        self.__book_dao.update_book(book_id, book)
        # 주문 생성
        order_id = str(OrderService.order_id_seq)
        OrderService.order_id_seq += 1
        order = Order(order_id, member_id, [OrderItem(book_id, quantity)], datetime.now(), OrderStatus.COMPLETED)
...
if __name__ == '__main__':
    ...
    order = os.create_order('woongseok', '1000', 2)
    print(order)
    print('파이썬 재고:', book_dao.select_book_by_id('1000').get_stock())  # 3
    try:
        os.create_order('woongseok', '1000', 99)
