    ...
    def menu_order_cart(self):
        ...
        for cart in cart_list:
            for item in cart.get_cart_items():
                try:
                    order = self.os.create_order(self.ms.current_user, item.get_book_id(), item.get_quantity())
                except BookstoreError as e:
                    print(f'[도서 {item.get_book_id()}] {e}') # 실패 -> 장바구니에 그대로 남김
                    continue
                delivery = self.ds.create_delivery(order.get_order_id(), self.ms.current_user, address)
                print(f'주문 완료 (주문번호 {order.get_order_id()}, ' f'배송번호 {delivery.get_delivery_id()})')
                ordered += 1
            self.cs.delete_cart(cart.get_cart_id()) # 주문 성공한 도서만 장바구니에서 제거
        if ordered:
